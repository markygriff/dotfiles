...
def process_args(args):
    ...
    # -r
    if args.r:
        # proc = subprocess.Popen([os.environ['WAL_DIR'] '/set_color.sh'], stdout=subprocess.PIPE)
    ...
    if args.i or args.f:
        ...
        # proc = subprocess.Popen([os.environ['WAL_DIR'] '/set_color.sh'], stdout=subprocess.PIPE)
        # proc = subprocess.Popen(['/Users/Marky/set_color.sh'], stdout=subprocess.PIPE)
        print("colors: Set terminal colors")

        export.export_all_templates(colors_plain)

        # no need, unless there is an environment to reset. For now, there is not
        # reload.reload_env()
